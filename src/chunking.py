...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if not remaining_separators:
            # No separator left to try: hard-split by character count.
            return [
        ...
        if separator == "":
            # Empty separator means "split anywhere": same as the hard-split fallback.
            return [
                current_text[i : i + self.chunk_size]
                for i in range(0, len(current_text), self.chunk_size)
            ]
        ...
        # Greedily re-merge parts up to chunk_size; recurse into any part that
        # is still too big on its own, using the next separator in priority order.
        chunks: list[str] = []
        buffer = ""
        for part in parts:
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        # TODO (đã code): call each chunker, compute stats, return comparison dict
        strategies = {
