...
from src.custom_chunking import PolicySectionChunker, contextual_chunk_document
...
def run_manual_demo(question: str | None = None, data_dir: str | None = None) -> int:
    ...
    # Mặc định dùng chiến lược thắng benchmark của nhóm (Hit@3 1.00 / MRR@3 0.87):
    # cắt theo điều khoản + gắn tiền tố ngữ cảnh. Đặt LAB_CHUNKER=fixed để so với baseline.
    if os.getenv("LAB_CHUNKER", "policy").strip().lower() == "fixed":
        store = build_knowledge_base(data_dir, embedding_fn=embedder)
    else:
        store = build_knowledge_base(
            data_dir,
            embedding_fn=embedder,
            chunker=PolicySectionChunker(chunk_size=500),
            chunk_builder=contextual_chunk_document,
        )
