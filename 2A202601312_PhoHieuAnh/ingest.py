...
def build_knowledge_base(
    ...
    `chunk_builder` cho phép thay bước "tài liệu -> danh sách chunk-Document";
    mặc định là `chunk_document`, truyền `contextual_chunk_document` nếu muốn
    gắn thêm tiền tố ngữ cảnh vào mỗi chunk trước khi embed.
    """
    chunker = chunker or FixedSizeChunker()
    chunk_builder = chunk_builder or chunk_document
    chunk_docs: list[Document] = []
    for doc in load_documents(data_dir):
        chunk_docs.extend(chunk_builder(doc, chunker))
